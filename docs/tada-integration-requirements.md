...
(`C:\Dev\Tada\backend`) found that its current API
...
|---|---|---|---|
| 1 | **No inbound device token.** Auth is a signed session cookie (`tada_session`) from PIN login only — no `Authorization` header / API-key path. | `app/services/auth_service.py`, `app/routers/auth.py` | A static **device token** (env, e.g. `HEARTH_API_TOKEN`) accepted as `Authorization: Bearer <token>` on a device-scoped router. It authorizes the *device*, not a user. **Its value must equal Hearth's `TADA_DEVICE_TOKEN`** — see the token model below. Don't name it `HEARTH_DEVICE_TOKEN`: that's an unrelated Hearth variable (the wall's own login token). |
...
### Token model — three separate secrets, don't conflate them

There are **three distinct tokens** in play. Only one is shared, and it is a
**newly generated secret**, not a copy of any existing token:

| Token | Set in | What it does | Shared with Tada!? |
|---|---|---|---|
| `HEARTH_DEVICE_TOKEN` | Hearth | Authorizes the **wall → Hearth** front door (`/setup?token=…` sets the device cookie). | **No** — never leaves Hearth. |
| `TADA_DEVICE_TOKEN` | Hearth | The secret Hearth sends to Tada! as `Authorization: Bearer …` on every `/api/hearth/*` call. | **Yes.** |
| `HEARTH_API_TOKEN` (name it whatever) | Tada! | What Tada! checks the incoming `Authorization: Bearer` against. | Must **equal Hearth's `TADA_DEVICE_TOKEN`**. |

To wire it up: `openssl rand -hex 32` **once** → set that value as Hearth's
`TADA_DEVICE_TOKEN` **and** as Tada!'s inbound token. **Do not** reuse Hearth's
`HEARTH_DEVICE_TOKEN` (the wall-login token) for this — different trust boundary,
and it isn't the value Hearth sends to Tada! anyway.
...
1. Generate **one new** shared secret (`openssl rand -hex 32`). Set it as Hearth's
   `TADA_DEVICE_TOKEN` **and** as Tada!'s inbound token (see the token model
   above — it is *not* Hearth's `HEARTH_DEVICE_TOKEN`). Set `TADA_API_URL` on
   Hearth to Tada!'s base URL.
