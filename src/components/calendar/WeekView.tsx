...
//
// Same two tap zones as the month cell (Phase 1.5 #10): the events open the day
// panel; the empty column area adds an event pre-filled to that day.
    ...
    <div className="grid h-full grid-cols-7 gap-3">
      {days.map((date) => {
        ...
        return (
          <div
            key={date.getTime()}
            className={`relative flex min-h-0 flex-col rounded-2xl px-3 pb-3 pt-3 ${
              today ? "bg-surface shadow-[inset_0_0_0_1px_var(--color-hairline)]" : ""
            }`}
          >
            {/* Empty-area tap adds an event on this day. */}
            <button
              type="button"
            ...
            <div className="pointer-events-none relative z-10 flex min-h-0 flex-col">
              ...
              {dayEvents.length === 0 ? (
                ...
                <button
                  ...
                  className={`pointer-events-auto flex min-h-0 flex-1 flex-col gap-2 overflow-hidden text-left ${
                    past ? "opacity-60" : ""
                  }`}
                >
                  {dayEvents.map((ev) => (
